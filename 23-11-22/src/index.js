...
const server = http.createServer((req, res) => {
  ...
  switch (myUrl) {
    case "/home":
      ...
      break;
    case "/calcolatrice":
      const data = fs.readFileSync(".src/html/calc.html");
      res.write(data.toString());
      break;
    case "/sum":
      const somma = fs.readFileSync(".src/html/sum.html");
      res.write(somma.toString());
      res.write(`${sum(params1, params2)}`);
      break;
    case "/sub":
      const sottrai = fs.readFileSync(".src/html/sub.html");
      res.write(sottrai.toString());
      res.write(`${sottrazione(params1, params2)}`);
      break;
    case "/moltiplicazione":
      const moltiplicazione = fs.readFileSync(".src/html/molti.html");
      res.write(moltiplicazione.toString());
      res.write(`${molti(params1, params2)}`);
      break;
    case "/divisione":
      const dividi = fs.readFileSync(".src/html/div.html");
